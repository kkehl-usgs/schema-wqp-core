CREATE OR REPLACE package body WQP_CORE.etl_helper_summary as
    ...
    begin
        ...
        begin
            execute immediate 'drop table org_sum_temp_' || p_table_suffix;
        exception
        ...
        begin
            execute immediate 'drop table year_sum_temp_' || p_table_suffix;
        exception
            when table_not_found then null;
            when others then raise;
        end;

        execute immediate 'create table org_sum_temp_' || p_table_suffix || q'! as
            select /*+ full(R) parallel(4,R) */
                   ...
                   count(distinct case when event_date >= trunc(sysdate, 'yyyy') then activity_id else null end) activity_count_current_year
              from result_swap_!' || p_table_suffix || ' R 
                group by data_source_id, organization';
        commit;

        select lock_value into l_lock_value from lock_table for update;
        execute immediate 'create table year_sum_temp_' || p_table_suffix || q'! as
            select /*+ noparallel(W) */
                   data_source_id,
                   organization,
              ...
              from (select /*+ noparallel(org_year_agg) */
                      ...
                      from (select /*+ noparallel(M) */
                              ...
                              from org_grouping_swap_!' || p_table_suffix || ' M
        ...
        sql_suffix := q'!((data_source_id, data_source, organization_id, organization, organization_name, organization_url,
            ...
            select /*+ leading(org_data org_sum year_summary) full(org_data) use_hash(org_data) full(org_sum) use_hash(org_sum) full(year_summary) use_hash(year_summary) */
              ...
              from (select data_source_id,
                           ...
                           organization_type
                      from org_data_swap_!' || p_table_suffix || ') org_data,
                   org_sum_temp_' || p_table_suffix || ' org_sum,
                   year_sum_temp_' || p_table_suffix || ' year_summary
    ...
    begin
     ...
     begin
         ...
         create_ml_grouping_idxs(p_table_suffix);
    ...
    begin
        ...
        dbms_output.put_line('ml_grouping');
        execute immediate 'alter table ml_grouping exchange partition ml_grouping_' || suffix ||
                          ' with table ml_grouping_swap_' || suffix || ' including indexes';

        dbms_output.put_line('org_grouping');
        execute immediate 'alter table org_grouping exchange partition org_grouping_' || suffix ||
                          ' with table org_grouping_swap_' || suffix || ' including indexes';
